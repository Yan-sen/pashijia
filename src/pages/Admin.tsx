...
function SpeciesTab({ pw }: { pw: string }) {
  const utils = trpc.useUtils();
  const [filterCat, setFilterCat] = useState("");
  const list = trpc.species.list.useQuery(filterCat ? { category: filterCat } : {});
  const cats = trpc.categories.list.useQuery();
  const invalidate = () => utils.species.list.invalidate();
  const update = trpc.species.update.useMutation({ onSuccess: invalidate });
  ...
  const save = () => {
    ...
    if (editingId) {
      update.mutate({ password: pw, id: editingId, data });
      setEditingId(null);
      setShowForm(false);
      setForm({ ...emptyForm });
    } else {
      if (!data.latinName || !data.groupName) {
        alert("拉丁学名和分组名必填");
        return;
      }
      create.mutate({ password: pw, data });
    }
  };
      ...
      {showForm && (
        <div className="mb-6 grid gap-3 border border-neutral-200 bg-[#f8f6ee] p-5 md:grid-cols-3">
          ...
          <div className="md:col-span-3">
            ...
            {editingId && (
              <button onClick={() => { setEditingId(null); setShowForm(false); setForm({ ...emptyForm }); }} className="ml-3 text-sm text-neutral-500 underline">
                取消编辑
              </button>
            )}
          </div>
            ...
            {(list.data ?? []).map((s) => (
              <tr key={s.id} className="border-t border-neutral-200">
                <td className="px-3 py-2">
                  <label className="block h-12 w-12 cursor-pointer overflow-hidden border border-dashed border-neutral-300 text-center text-[10px] leading-[3rem] text-neutral-400 hover:border-[#c9a227]">
                    {s.imageUrl ? (
                ...
                <td className="px-3 py-2">
                  <select
                    value={s.category}
                    onChange={(e) => update.mutate({ password: pw, id: s.id, data: { category: e.target.value } })}
                  ...
                  <input
                    ...
                    onBlur={(e) => {
                      if (e.target.value !== (s.priceUsd ?? ""))
                        update.mutate({ password: pw, id: s.id, data: { priceUsd: e.target.value || null } });
                    }}
                    className={`${inp} w-24`}
                  />
                </td>
                <td className="px-3 py-2">
                  <input
                    ...
                    onBlur={(e) => {
                      ...
                      if (v !== s.stock) update.mutate({ password: pw, id: s.id, data: { stock: v } });
                    }}
                    className={`${inp} w-16`}
                  />
                </td>
                <td className="px-3 py-2">
                  <input
                    type="checkbox"
                    checked={s.showPrice}
                    onChange={(e) => update.mutate({ password: pw, id: s.id, data: { showPrice: e.target.checked } })}
                ...
                <td className="px-3 py-2">
                  <input
                    type="checkbox"
                    checked={s.featured}
                    onChange={(e) => update.mutate({ password: pw, id: s.id, data: { featured: e.target.checked } })}
                    className="h-4 w-4 accent-[#c9a227]"
                  />
                </td>
                <td className="px-3 py-2">
